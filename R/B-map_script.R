...
ConductivityCBV <- as.factor(c(rep(mean_spCond_cbv[1], 2), rep(mean_spCond_cbv[2],2), rep(mean_spCond_cbv[3], 4), rep(mean_spCond_cbv[5],3)))

siteName <- as.factor(c(rep("Claybank", 2), rep("Goodwin Islands",2), rep("Sweethall",4), rep("Taskinas Creek", 3)))
...
ConductivityOWC <- as.factor(c(rep(mean_spCond_owc[1], 2), rep(mean_spCond_owc[2],2), rep(mean_spCond_owc[3], 2), rep(mean_spCond_owc[4],2), rep(mean_spCond_owc[5],2)))
siteName <- as.factor(c(rep("Berlin Road", 2), rep("Darrow Road",2), rep("Lower Estuary", 2), rep("State Route 2",2), rep("State Route 6",2)))
